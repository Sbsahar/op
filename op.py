...
DEVELOPER_ID = 6789179634  # رقمك كمطور
...
# تحديث إحصائيات القنوات عند الاشتراك الناجح
def update_channel_stats():
    cursor.execute('SELECT channel_username FROM mandatory_channels')
    channels = cursor.fetchall()
    for channel in channels:
        cursor.execute('UPDATE channel_stats SET subscribers_count = subscribers_count + 1 WHERE channel_username = ?', (channel[0],))
    conn.commit()
...
def start(message):
    ...
    update_channel_stats()  # تحديث الإحصائيات بعد الاشتراك
...
def show_developer_panel(message):
    ...
    bot.send_message(message.chat.id, "<b>لوحة التحكم للمطور:</b>", parse_mode='HTML', reply_markup=keyboard)
...
def callback_handler(call):
    ...
    if data == "add_service":
        ...
        bot.edit_message_text("<b>اختر فئة الخدمة الجديدة:</b>", call.message.chat.id, call.message.id, parse_mode='HTML', reply_markup=keyboard)
        ...
        if check_subscription(user_id):
            ...
            bot.answer_callback_query(call.id, "ما زلت غير مشترك. اشترك أولاً!", show_alert=True)
        ...
        if state == 'quantity':
            try:
                ...
                if quantity < min_q or quantity > max_q:
                    ...
                    return
                user_states[user_id] = {'state': 'link', 'service': service, 'quantity': quantity}
        ...
        # حالات المطور
            ...
            try:
                ...
                if is_admin:
                    ...
                    bot.reply_to(message, "<b>البوت ليس مشرفاً في القناة أو لا يملك صلاحية فحص الأعضاء.</b>", parse_mode='HTML')
            except:
                bot.reply_to(message, "<b>خطأ في التحقق من القناة.</b>", parse_mode='HTML')
            del user_states[user_id]

# تأكيد خصم الرصيد (من callback)
