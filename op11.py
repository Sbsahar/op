...
def check_and_award_referral(user_id, referred_by):
    ...
    cursor = conn.cursor()
    cursor.execute('SELECT verified_steps, has_used_multi_code FROM users WHERE user_id = ?', (user_id,))
    ...
    # التحقق: هل أكمل الخطوتين + لم يستخدم كود متعدد؟
    if set(['daily_gift', 'services']).issubset(set(verified_steps)) and has_used_multi_code == 0:
        def award_points():
            try:
                cursor = conn.cursor()
    ...
    # إذا استخدم كود متعدد، لا نمنح نقاط الدعوة (حتى لو أكمل الخطوات)
