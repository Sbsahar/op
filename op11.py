...
def clean_database():
    ...
    # إضافة عمود جديد: هل استخدم المستخدم كودًا متعددًا؟
    try:
...
def start(message):
    ...
    chat_type = message.chat.type  # نوع المحادثة: private, group, supergroup

    # إذا كانت الرسالة في مجموعة (عامة أو خارقة)
...
def check_and_award_referral(user_id, referred_by):
    ...
    # جلب البيانات: الخطوات + هل استخدم كود متعدد + إجمالي الشحن
    cursor.execute('''
        SELECT verified_steps, has_used_multi_code, total_charged 
        FROM users 
        WHERE user_id = ?
    ''', (user_id,))
    ...
    has_used_multi_code = result[1] if result[1] is not None else 0
    total_charged = result[2] if result[2] is not None else 0

    # الشرط: أكمل الخطوتين + لم يستخدم كود متعدد + شحن نقاط (كود فريد)
    if (set(['daily_gift', 'services']).issubset(set(verified_steps)) 
        and has_used_multi_code == 0 
        and total_charged > 0):  # ← هذا هو السحر!
        ...
        def award_points():
            try:
                ...
                bot.send_message(user_id, "<b>مبروك! حصلت على 5 نقاط إضافية، وحصل داعيك على 50 نقطة!</b>", parse_mode='HTML')
                try:
                    bot.send_message(referred_by, "<b>مبروك! حصلت على 50 نقطة لأن صديقك قام بالشحن!</b>", parse_mode='HTML')
                except:
                    pass
            ...
            if code_info and code_info[1] == 0:
                ...
                if multi_code_info and multi_code_info[3] == 'active':
                    ...
                    if cursor.fetchone():
                        ...
                        # تم التعديل هنا: إضافة has_used_multi_code = 1
