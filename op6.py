...
def check_subscription(user_id):
    ...
    for channel in channels:
        cursor.execute('SELECT * FROM user_subscriptions WHERE user_id = ? AND channel_username = ?', (user_id, channel[0]))
        if cursor.fetchone():
            continue  # مشترك سابقًا أو حاليًا (نعتمد على التحقق الفعلي)
        try:
...
def show_mandatory_channels(message, from_callback=False):
    ...
    keyboard.add(InlineKeyboardButton("🔄 تحقق من الاشتراك الآن", callback_data="retry_subscription"))
    msg_text = "<b>عذرًا عزيزي 💖، أنت غير مشترك في بعض قنواتنا الرائعة! 🌟\nاشترك الآن واحصل على هدية بسيطة: 2 نقاط مجانية لكل قناة تشترك فيها مع بوت فولو ميديا! 🎁\nهذه النقاط تساعدك في الاستمتاع بخدماتنا المميزة بكل سهولة. 😊\n\nاشترك بالقنوات التالية 👇🏻 واضغط على 'تحقق من الاشتراك الآن' للحصول على نقاطك:</b>"
    if from_callback:
...
def callback_handler(call):
    ...
    if user_id != DEVELOPER_ID and not check_subscription(user_id) and data not in ["retry_subscription"]:
...
"""
        ...
        # إعادة استدعاء start لضمان عرض الرصيد والـID الصحيحين
        start(call.message)
        ...
        if user_id in user_states and 'link' in user_states[user_id]:
            ...
            api_response = {}
            api_order_id = None
            if "مجانية" in category:  # الخدمات المجانية لا ترسل إلى API
                ...
                # إرسال الطلب إلى الـ API للخدمات غير المجانية
    ...
    if data == "add_service":
        ...
        if check_subscription(user_id):
            if points_added > 0:
                bot.edit_message_text(f"<b>شكرًا لك على اشتراكك! 🎉\nأنت اشتركت في {new_subs} قناة جديدة وحصلت على {points_added} نقاط هدية! 💎</b>", call.message.chat.id, call.message.id, parse_mode='HTML')
            bot.delete_message(call.message.chat.id, call.message.id)
            start(call.message)
        else:
            show_mandatory_channels(call.message, from_callback=True)
            bot.answer_callback_query(call.id, "ما زلت غير مشترك في بعض القنوات. اشترك أولاً! ⚠️", show_alert=True)
