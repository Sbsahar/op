...
@bot.callback_query_handler(func=lambda call: call.data == "retry_subscription")
def retry_subscription_callback(call):
    user_id = call.from_user.id
    if check_subscription(user_id):
        ...
        for channel in channels:
            try:
                member = bot.get_chat_member(f'@{channel[0]}', user_id)
                if member.status in ['left', 'kicked']:
                    unsubscribed.append(f"@{channel[0]}")
            except:
                unsubscribed.append(f"@{channel[0]}")
        if unsubscribed:
            msg = "<b>ما زلت غير مشترك في القنوات التالية:</b>\n" + "\n".join(unsubscribed) + "\n\n<b>اشترك ثم اضغط تحقق من الاشتراك.</b>"
            keyboard = InlineKeyboardMarkup()
            for channel in channels:
                keyboard.add(InlineKeyboardButton(f"اشترك في @{channel[0]}", url=f"https://t.me/{channel[0]}"))
            ...
            bot.answer_callback_query(call.id, "خطأ أثناء التحقق من الاشتراك. حاول مرة أخرى! ⚠️", show_alert=True)        
