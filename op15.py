...
DEVELOPER_ID = 6177743981  # ايدي المطور
...
def clean_database():
    cursor = conn.cursor()
    try:
        # حذف جدول referrals إذا كان موجودًا (تنظيف قديم)
        cursor.execute('DROP TABLE IF EXISTS referrals')
    except sqlite3.OperationalError:
        pass

    # الأعمدة الأساسية المطلوبة في جدول users
    try:
    ...
    try:
        cursor.execute('ALTER TABLE users ADD COLUMN has_used_multi_code INTEGER DEFAULT 0')
    except sqlite3.OperationalError:
        pass

    # العمود الجديد لمنع تكرار منح الهدايا
    try:
        cursor.execute('ALTER TABLE users ADD COLUMN awarded_referral INTEGER DEFAULT 0')
    except sqlite3.OperationalError:
        pass
...
def create_tables():
    ...
    cursor.execute('''CREATE TABLE IF NOT EXISTS multi_codes (
        code TEXT PRIMARY KEY,
        value INTEGER,
        max_users INTEGER,
    ...
    cursor.execute('''CREATE TABLE IF NOT EXISTS multi_code_usages (
        user_id INTEGER,
        code TEXT,
...
# دالة لإرسال رسالة الستارت مع ايدي المستخدم الصحيح
def send_start_message(user_id, chat_id):
    cursor = conn.cursor()
    ...
    keyboard.add(
        ...
        InlineKeyboardButton(" استخدام كود🎟️", callback_data="use_code")
    )
    keyboard.add(
...
def start(message):
    ...
    if chat_type in ['group', 'supergroup']:
        bot.reply_to(message, "<b>بوت فولو ميديا دائمًا بخدمكم ❤️</b>", parse_mode='HTML')
        return
    ...
    if not user:
        # مستخدم جديد → نحفظ referred_by من أول لحظة
        cursor.execute('''
            INSERT INTO users (user_id, balance, total_charged, total_orders, referred_by, verified_steps, has_used_multi_code)
            VALUES (?, 0, 0, 0, ?, "", 0)
        ''', (user_id, referred_by))
        conn.commit()
        balance = 0
    else:
        balance, current_referred_by = user
        # إذا كان referred_by جديدًا ولم يُحدد من قبل
        if referred_by and not current_referred_by:
            cursor.execute('UPDATE users SET referred_by = ? WHERE user_id = ?', (referred_by, user_id))
            conn.commit()

    # === لا نستخدم user_states أو pending_referral أبدًا ===
    ...
    send_start_message(user_id, message.chat.id)
...
def show_developer_panel(message):
    ...
    keyboard.add(
        ...
        InlineKeyboardButton("انشاء كود متعدد", callback_data="create_multi_code")
    )
    keyboard.add(
...
def check_and_award_referral(user_id, referred_by=None):
    with db_lock:  # استخدام القفل لحماية العمليات
        cursor = conn.cursor()
        if not referred_by:
        ...
        cursor.execute('SELECT has_used_multi_code, total_charged, awarded_referral FROM users WHERE user_id = ?', (user_id,))
        user_data = cursor.fetchone()
        if not user_data:
            return
        has_used_multi_code, total_charged, awarded_referral = user_data

        
        if has_used_multi_code == 0 and total_charged > 0 and awarded_referral == 0:
            try:
                # منح فوري
                ...
                cursor.execute('UPDATE users SET awarded_referral = 1 WHERE user_id = ?', (user_id,))
                conn.commit()

                bot.send_message(user_id, "<b>مبروك! حصلت على 5 نقاط إضافية كهدية!</b>", parse_mode='HTML')
                bot.send_message(referred_by, "<b>لقد حصلت على 50 كهدية بسيطة لك بعد أن قام أحد الأشخاص الذين دعوتهم بشراء نقاط من المطور! 🎁\n\nشكرًا لك على دعوة المستخدمين، تابع دعوة المزيد من المستخدمين واحصل على المزيد من الهدايا من فولو ميديا 🥰</b>", parse_mode='HTML')
...
"""
        ...
        cursor = conn.cursor()
        cursor.execute('SELECT last_daily_gift, referred_by FROM users WHERE user_id = ?', (user_id,))
        result = cursor.fetchone()
        last_gift, referred_by = result
        now = datetime.now()
        if last_gift:
            ...
            if now - last_gift_time < timedelta(hours=24):
                ...
                try:
                    ...
                    if "query is too old" in str(e):
                        pass
                return
        with db_lock:  # حماية تحديث قاعدة البيانات
...
- الأعضاء حقيقيون 100% من مستخدمي البوت اي تمويل حقيقي ✓.
- للتمويل، رفع البوت مشرفًا في قناتك أو مجموعتك مع صلاحيات دعوة المستخدمين.
- القناة أو المجموعة يجب أن تكون عامة برابط عام (@mediafolo).
- أقل تمويل: 40 عضو.

<b>ملاحظة هامة:</b> لا تقم بإزالة البوت من المشرفين قبل انتهاء التمويل، وإلا سيتم إلغاء التمويل ولايوجد تعويض.

<b>ابدأ تمويلك الآن!</b>
"""
        keyboard = InlineKeyboardMarkup()
        keyboard.add(InlineKeyboardButton("اضف تمويلك الان 👥", callback_data="add_funding"))
        keyboard.add(InlineKeyboardButton("تمويلاتي 📋", callback_data="my_fundings"))
        keyboard.add(InlineKeyboardButton("↩️ رجوع", callback_data="back_to_start"))
        bot.edit_message_text(msg, call.message.chat.id, call.message.id, parse_mode='HTML', reply_markup=keyboard)
        ...
        bot.edit_message_text(f"<b>نقاطك: {balance} 💎\nأقصى عدد يمكنك تمويله: {max_members} عضو\nأرسل العدد المطلوب (أقل {min_members}):</b>", call.message.chat.id, call.message.id, parse_mode='HTML', reply_markup=keyboard)

    elif data == "my_fundings":
        cursor = conn.cursor()
        cursor.execute('SELECT channel_username, requested_members, remaining_members FROM fundings WHERE user_id = ? AND status = "active"', (user_id,))
        fundings = cursor.fetchall()
        if not fundings:
            msg = "<b>ليس لديك تمويلات نشطة حالياً.</b>"
        else:
            msg = "<b>تمويلاتك النشطة:</b>\n"
            for funding in fundings:
                channel, requested, remaining = funding
                msg += f"القناة: @{channel}\nالمطلوب: {requested} عضو\nالمتبقي: {remaining} عضو\n\n"
        keyboard = InlineKeyboardMarkup()
        keyboard.add(InlineKeyboardButton("↩️ رجوع", callback_data="funding_members"))
        bot.edit_message_text(msg, call.message.chat.id, call.message.id, parse_mode='HTML', reply_markup=keyboard)
...
- القنوات المعروضة هنا بلبوت يجب ان تنضم اليها.
- ملاحظة بسيطة اي مغادرة للقنوات يعني خصم لنقاطك لذلك لاتغادر عزيزي!
...
"""
        ...
        if awarded:
            ...
            show_next_channel(call.message.chat.id, user_id)
        else:
        ...
        bot.delete_message(call.message.chat.id, call.message.id)
        bot.send_message(call.message.chat.id, "<b>تم التخطي! جاري عرض قناة جديدة... ⏳</b>", parse_mode='HTML')
        time.sleep(1)
        show_next_channel(call.message.chat.id, user_id)

    elif data == "back_to_start":
        bot.delete_message(call.message.chat.id, call.message.id)
        send_start_message(user_id, call.message.chat.id)
        ...
        if user_id in user_states and 'channel_username' in user_states[user_id]:
            ...
            if result == "added":
                bot.edit_message_text("<b>تم إضافة التمويل بنجاح ✅جاري عرض قناتك في البوت.</b>", call.message.chat.id, call.message.id, parse_mode='HTML')
            elif result == "queue":
...
# تأكيد الطلب من المجموعة
    ...
    if data == "add_service":
        ...
        bot.edit_message_text(
            f"<b>هل ترغب بحذف الخدمة '{service_name}' نهائياً ❓</b>",
            call.message.chat.id,
            call.message.id,
            parse_mode='HTML',
            reply_markup=keyboard
        ...
        keyboard.add(InlineKeyboardButton("رجوع", callback_data="dev_back"))
        bot.edit_message_text(
            "<b>ارسل عدد النقاط التي تريد إضافتها (مثل: 1000) 💵:</b>",
            call.message.chat.id,
            call.message.id,
            parse_mode='HTML',
            reply_markup=keyboard
    ...
    elif data == "create_multi_code":
        user_states[user_id] = {'state': 'multi_code_max_users'}
        keyboard = InlineKeyboardMarkup()
        keyboard.add(InlineKeyboardButton("رجوع", callback_data="dev_back"))
        bot.edit_message_text(
            "<b>ارسل عدد الأشخاص الذين يمكنهم استخدام الكود (مثل: 50) 👥:</b>",
            call.message.chat.id,
            call.message.id,
            parse_mode='HTML',
            reply_markup=keyboard
...
def handle_text(message):
    # تجاهل الرسائل في المجموعات (عامة أو خارقة)
    if message.chat.type in ['group', 'supergroup']:
        return  # لا يتم الرد على الرسائل في المجموعات
    ...
    if user_id in user_states:
        ...
        if state == 'create_code_value':
        ...
        elif state == 'multi_code_max_users':
            try:
                max_users = int(text)
                if max_users <= 0:
                    bot.reply_to(message, "<b>أدخل عددًا إيجابيًا صحيحًا.</b>", parse_mode='HTML')
                    return
                user_states[user_id]['max_users'] = max_users
                user_states[user_id]['state'] = 'multi_code_value'
                keyboard = InlineKeyboardMarkup()
                keyboard.add(InlineKeyboardButton("رجوع", callback_data="dev_back"))
                bot.reply_to(message, "<b>ارسل عدد النقاط لكل مستخدم (مثل: 10):</b>", parse_mode='HTML', reply_markup=keyboard)
            except:
                bot.reply_to(message, "<b>أدخل رقم صحيح.</b>", parse_mode='HTML')
        
        elif state == 'multi_code_value':
            try:
                value = int(text)
                if value <= 0:
                    bot.reply_to(message, "<b>أدخل عددًا إيجابيًا صحيحًا.</b>", parse_mode='HTML')
                    return
                max_users = user_states[user_id]['max_users']
                code = ''.join(random.choices(string.ascii_uppercase + string.digits, k=6))
                with db_lock:
                    cursor = conn.cursor()
                    cursor.execute('INSERT INTO multi_codes (code, value, max_users, used_count, status) VALUES (?, ?, ?, 0, "active")', (code, value, max_users))
                    conn.commit()
                bot.reply_to(message, f"<b>تم إنشاء الكود: {code}\nقيمته: {value} نقطة\nعدد المستخدمين المسموح: {max_users}\nالحالة: نشط</b>", parse_mode='HTML')
                del user_states[user_id]
            except:
                bot.reply_to(message, "<b>أدخل رقم صحيح.</b>", parse_mode='HTML')
            ...
            try:
                ...
                if quantity < min_q or quantity > max_q:
                    bot.reply_to(message, f"<b>الكمية يجب أن تكون بين {min_q} و {max_q}.</b>", parse_mode='HTML')
                    return
            ...
            if int(text) == user_states[user_id]['captcha_code']:
                ...
                with db_lock:
                    ...
                    if balance < total_price:
                        bot.reply_to(message, "<b>رصيدك غير كافي.</b>", parse_mode='HTML')
                        del user_states[user_id]
                        start(message)
                        return
                ...
                bot.send_message(message.chat.id, "<b>شكرا لاستخدامك بوت فولو ميديا</b>", parse_mode='HTML')
                del user_states[user_id]
                start(message)
            else:
            ...
            with db_lock:
                code = text.upper()
                cursor = conn.cursor()
                # التحقق من الكود الفريد أولاً
                cursor.execute('SELECT value, used FROM codes WHERE code = ?', (code,))
                code_info = cursor.fetchone()
                if code_info and code_info[1] == 0:
                    value = code_info[0]
                    cursor.execute('UPDATE users SET balance = balance + ?, total_charged = total_charged + ?, has_used_multi_code = 0 WHERE user_id = ?', (value, value, user_id))
                    ...
                    # التحقق من الكود المتعدد
                    cursor.execute('SELECT value, max_users, used_count, status FROM multi_codes WHERE code = ?', (code,))
                    multi_code_info = cursor.fetchone()
                    if multi_code_info and multi_code_info[3] == 'active':
                        value, max_users, used_count, status = multi_code_info
                        cursor.execute('SELECT * FROM multi_code_usages WHERE user_id = ? AND code = ?', (user_id, code))
                        if cursor.fetchone():
                            bot.reply_to(message, "<b>لقد استخدمت هذا الكود سابقًا.</b>", parse_mode='HTML')
                        elif used_count >= max_users:
                            cursor.execute('UPDATE multi_codes SET status = "completed" WHERE code = ?', (code,))
                            conn.commit()
                            bot.reply_to(message, "<b>هذا الكود مكتمل العدد ولا يمكن استخدامه بعد الآن.</b>", parse_mode='HTML')
                        else:
                            cursor.execute('UPDATE users SET balance = balance + ?, total_charged = total_charged + ?, has_used_multi_code = 1 WHERE user_id = ?', (value, value, user_id))
                            cursor.execute('UPDATE multi_codes SET used_count = used_count + 1 WHERE code = ?', (code,))
                            cursor.execute('INSERT INTO multi_code_usages (user_id, code) VALUES (?, ?)', (user_id, code))
                            conn.commit()
                            new_used_count = used_count + 1
                            if new_used_count >= max_users:
                                cursor.execute('UPDATE multi_codes SET status = "completed" WHERE code = ?', (code,))
                                conn.commit()
                            bot.reply_to(message, f"<b>تم إضافة {value} نقطة إلى رصيدك بنجاح!</b>", parse_mode='HTML')
                    else:
            ...
            try:
                max_users = int(text)
                if max_users <= 0:
                    bot.reply_to(message, "<b>أدخل عددًا إيجابيًا صحيح ❌.</b>", parse_mode='HTML')
                    return
                user_states[user_id]['max_users'] = max_users
                user_states[user_id]['state'] = 'multi_code_value'
                keyboard = InlineKeyboardMarkup()
    ...
    try:
        ...
        if member.status not in ['left', 'kicked'] and not cursor.execute('SELECT 1 FROM user_join_history WHERE user_id = ? AND funding_id = ?', (user_id, funding_id)).fetchone():
            ...
            if remaining <= 0:
                ...
                conn.commit()
                bot.send_message(owner_id, f"<b>تم الانتهاء من تمويل قناتك @{channel_username} بنجاح ✅!\nلا تقم بإزالة البوت من المشرفين لضمان عدم المغادرة.</b>", parse_mode='HTML')
            return True
    except Exception as e:
        print(f"Error in check_and_award_join: {str(e)}")
    return False
# عرض القناة التالية للمستخدم
def show_next_channel(chat_id, user_id):
    channel = get_next_channel_for_user(user_id)
    if not channel:
        bot.send_message(chat_id, "<b>لا توجد قنوات متاحة الآن. أعد المحاولة لاحقًا! ⏳</b>", parse_mode='HTML')
        return
    msg = f"<b>اشترك في @{channel['username']} واحصل على 5 نقاط! 👥</b>"
    keyboard = InlineKeyboardMarkup()
    keyboard.add(InlineKeyboardButton("اضغط لاشتراك 📢", url=f"https://t.me/{channel['username']}"))
    ...
    while True:
        ...
        for funding in fundings:
            ...
            try:
                ...
                print(f"Error in check_admin_status: {str(e)}")
