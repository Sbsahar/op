...
def start(message):
    ...
    cursor.execute('INSERT OR IGNORE INTO users (user_id, balance, total_charged, total_orders) VALUES (?, 0, 0, 0)', (user_id,))
    if referred_by:
...
def check_and_award_referral(user_id, referred_by):
    if referred_by:
        ...
        verified_steps = cursor.fetchone()[0].split(',') if cursor.fetchone()[0] else []
        if set(['daily_gift', 'services']).issubset(set(verified_steps)):
            def award_points():
                ...
                # مسح verified_steps لمنع التكرار
...
def callback_handler(call):
    ...
    if data == "services":
        ...
        # إضافة 'services' إلى verified_steps
