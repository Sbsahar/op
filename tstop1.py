...
"""
        ...
        from funding import get_next_channel_for_user  # استيراد من الملف المنفصل
        ...
        from funding import check_and_award_join
    ...
    if data == "add_service":
        ...
        from funding import get_active_fundings
        ...
        from funding import get_funding_details, remove_funding
        ...
        from funding import remove_funding
...
def handle_text(message):
    ...
    if user_id in user_states:
        ...
        # حالات المطور
            ...
            text = text.strip()  # إضافة strip لإزالة المسافات
            try:
                target_id = int(text)
                cursor.execute('SELECT balance FROM users WHERE user_id = ?', (target_id,))
                if cursor.fetchone():
            ...
            text = text.strip()
            if 't.me/' in text:
                channel_username = text.split('t.me/')[1].strip('/').lstrip('@')
            else:
            ...
            try:
                ...
                if is_admin:
                    from funding import add_funding_request
                    quantity = user_states[user_id]['quantity']
                    cost = quantity * 8
                    cursor.execute('SELECT balance FROM users WHERE user_id = ?', (user_id,))
                    balance = cursor.fetchone()[0]
