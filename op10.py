...
"""
        ...
        bot.delete_message(call.message.chat.id, call.message.id)
        # Use call.from_user.id to get the correct user ID
        send_start_message(call.from_user.id, call.message.chat.id)
  ...
  # تأكيد الطلب من المجموعة
    ...
    if data == "add_service":
        ...
        keyboard.add(InlineKeyboardButton("رجوع", callback_data="dev_back"))
        msg = "<b>قائمة التمويلات النشطة 👥:</b>\n" if fundings else "<b>لا توجد تمويلات نشطة حاليًا.</b>"
        if fundings:
            for funding in fundings:
                msg += f"@{funding['channel_username']}\n"
        bot.edit_message_text(msg, call.message.chat.id, call.message.id, parse_mode='HTML', reply_markup=keyboard)
